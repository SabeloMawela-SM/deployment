...
form.addEventListener("submit",  (e) => {
  ...
  let netWithdrawal =(withdrawalAmount - estimatedTax)- estimatedTransition;

  // Future value calc with inflation and growth
  let savingsGrowth = savingsComponentValue ;
  for (let i = 0; i < yearsToRetirement; i++) {
    savingsGrowth = ( savingsComponentValue + (monthlyContribution  * 12) * Math.pow(1 + realRate, i));
  
  }
  let twoPot = savingsGrowth * (1/3);

  let futureAfter = savingsGrowth - ((twoPot/3) - savingsComponentValue);
  ...
  container.innerHTML = `
    ...
    <div class="mt-4 m-4">
      <h5>Current value of savings component:</h5>
        ...
        <div class="progress">
          <div class="progress-bar bg-primary" role="progressbar" style="width: ${(savingsComponentValue )* 100}%"></div>
        </div>
        <p>After withdrawal: <strong>R ${(savingsComponentValue - withdrawalAmount).toFixed(2)}</strong></p>
        <div class="progress mb-3">
          <div class="progress-bar bg-primary" role="progressbar" style="width: ${(savingsComponentValue - withdrawalAmount) / savingsComponentValue * 100}%"></div>
        </div>
      ...
      <div class="progress">
        <div class="progress-bar bg-primary" role="progressbar" style="width: ${( savingsGrowth) * 100}%"></div>
      </div>
      <p>After withdrawal: <strong>R ${futureAfter.toFixed(2)}</strong></p>
      <div class="progress">
        <div class="progress-bar bg-primary" role="progressbar" style="width: ${(futureAfter / savingsGrowth) * 100}%"></div>
      </div>
    </div>
